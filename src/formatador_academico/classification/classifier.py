...
def project_run_classification(
    ...
    # Parent binding (audit item 23): inheritance is only sound if the run is
    # physically a descendant of the classified paragraph. The parser v0.4
    # structural_path is strictly hierarchical (a run path nests below its
    # paragraph path), so the prefix check proves the parent-child relation
    # without reparsing anything. The "/" separator prevents false prefix
    # matches such as /w:p[1] vs /w:p[10]. Passing a run of paragraph A with
    # the classification of paragraph B is a contract/programming error and
    # raises, never silently inherits a foreign class.
    parent_path = paragraph_result.structural_path
    if not run_path.startswith(parent_path + "/"):
        raise ValueError(
            "run is not physically contained in the classified paragraph "
            f"(run path {run_path!r} is not under paragraph path {parent_path!r}); "
            "inheritance requires proven parent-child binding"
        )
