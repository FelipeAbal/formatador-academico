...
def _canonical_xml(node: etree._Element) -> bytes:
    # Standalone comment/PI nodes are serialized directly; libxml2 C14N on them can be unsafe.
...
def _structural_path(node: etree._Element, root: etree._Element) -> str:
    """1-based position among same-kind/same-tag siblings; root has no predicate."""
    ...
    for element in reversed(elements):
        if not isinstance(element.tag, str):
            name = "comment()" if isinstance(element, etree._Comment) else "processing-instruction()"
        else:
        ...
        for child in parent:
            if isinstance(element.tag, str):
                ...
                same_kind.append(child)
        index = same_kind.index(element) + 1
            ...
            with zipfile.ZipFile(io.BytesIO(docx_bytes), mode="r") as zf:
                ...
                if root_local != "document" or root_ns != W_NS:
                    if root_ns == STRICT_W_NS:
                        raise ParseFailure("unsupported_namespace", f"Strict OOXML namespace is not supported in v0.1: {root_ns}")
                ...
                for original_index, child in enumerate(body):
                    ...
                    if not isinstance(child.tag, str):
                        ...
                        if namespace == W_NS and local == "p":
                            source_type, protected = "paragraph", False
                        elif namespace == W_NS and local == "tbl":
                            source_type, protected = "table", False
                            warnings.append({"code": "unparsed_children", "message": "Table interior is preserved as canonical XML but not decomposed in parser v0.1.", "structural_path": path})
