...
def _has_xml_declaration(original_bytes: bytes) -> bool:
    """Detect whether the original byte stream contained an XML declaration.

    XML declarations are ASCII text in ASCII-compatible encodings, but UTF-16
    and UTF-32 streams encode those characters with NUL bytes. Detection is
    therefore encoding-aware and never infers declaration presence merely
    from a BOM.
    """

    data = original_bytes
    if data.startswith(b"\xef\xbb\xbf"):
        return data[3:].lstrip().startswith(b"<?xml")
    if data.startswith((b"\xff\xfe\x00\x00", b"\x00\x00\xfe\xff")):
        try:
            return data.decode("utf-32").lstrip().startswith("<?xml")
        except UnicodeDecodeError:
            return False
    if data.startswith((b"\xff\xfe", b"\xfe\xff")):
        try:
            return data.decode("utf-16").lstrip().startswith("<?xml")
        except UnicodeDecodeError:
            return False
    # BOM-less UTF-16 signatures allowed by XML processors.
    if data.startswith((b"<\x00?\x00x\x00m\x00l\x00", b"\x00<\x00?\x00x\x00m\x00l")):
        return True
